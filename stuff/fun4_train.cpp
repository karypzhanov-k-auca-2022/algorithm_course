...
public:
    ...
    std::shared_ptr<Mine> selectMine(int day) {
        ...
        for (const auto &mine: mines) {
            ...
            if (mine->getDaysWorked() == 0) {
                return mine;
            }

            double ucb = mine->getTotalExtractedGold() / mine->getDaysWorked() +
                         sqrt(2 * log(day + 1) / mine->getDaysWorked());

            if (ucb > maxUCB) { // select the mine with the highest UCB value
                maxUCB = ucb; // update  maximum UCB
                selectedMine = mine; // update mine
            }
        }
    ...
    void exploitMines(int day) {
        ...
        if (selectedMine && !selectedMine->isUnderThreat()) {
            totalEarned += selectedMine->extractGold();  // extract gold from the mine
        }
...
int main() {
    ...
    double winRate = (double) wins / howManyGames * 100;
    std::cout << "Wins: " << wins << " out of " << howManyGames << " games" << std::endl;
